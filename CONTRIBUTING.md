...
KittyLM is currently a private, single-maintainer research project. This document defines how
...
- **GitHub Actions is authoritative.** All required CI jobs must pass. If branch protection with
  required status checks is unavailable for this private repository, that rule is followed by
  convention.
...
- **Documentation rule:** every non-trivial subsystem explains what it does *and why*. Modules
  ...
  `Purpose:`, `Public API:`, `Invariants:`, `Failure modes:`, `See:` sections, plus `Shapes:`,
  `Dtype:`, `Device:` for modules that import torch. Enforced by `tests/test_docs_contract.py`.
