"""Run KittyLM checks. GitHub Actions calls exactly these groups, so local == CI.
...
def report_prepublication() -> int:
    result = subprocess.run(
        ["git", "grep", "-n", "PRE-PUBLICATION", "--", ".", ":!scripts/check.py"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    if hits:
        print(f"  WARNING: {len(hits)} PRE-PUBLICATION item(s) must be resolved before any")
        print("  visibility change (docs/pre-publication.md). This does not fail the check:")
        for line in hits:
            print(f"    {line[:160]}")
    else:
        print("  no PRE-PUBLICATION markers")
    return 0
...
def check_provenance() -> int:
    ...
    if recipes or manifests:
        # Recipe/manifest validation arrives with the dataset tooling (plan step 3). Until it
        # exists, refuse rather than silently pass a dataset that nothing has validated.
...
def check_audit() -> int:
    # --local: in a venv created with --system-site-packages, audit what this project
    # installed rather than unrelated system packages. In CI (no venv) it audits everything.
    return _run("-m", "pip_audit", "--local", "--skip-editable", "--progress-spinner", "off")
...
GROUPS: dict[str, list[str]] = {
    "quality": ["format", "lint", "types", "configs", "imports", "structure", "ledger", "prepub"],
    "tests": ["pytest"],
