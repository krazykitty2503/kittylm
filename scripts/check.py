...
python scripts/check.py quality      # format, lint, types, configs, imports, structure, ledger, package
...
print(
    f"  package build: {len(wheels)} wheel(s), "
    f"{len(sdists)} source distribution(s)"
)
...
print(
    f"  artifact guard: {len(entries)} {scope} file(s), "
...
if recipes or manifests:
    ...
    print(
        "  data recipes/manifests exist but provenance validation "
...
if "No dataset has been built yet." not in data_sources:
    print(
        "  DATA_SOURCES.md must state that no dataset has been built "
        "(no manifests exist)"
    )
    return 1

print(
    "  provenance: 0 recipes, 0 manifests; "
    "DATA_SOURCES.md sync check inactive until step 3"
)
return 0
```
